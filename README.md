...
2. 当前消息中发送的图片/表情图片。
3. 当前消息中真正 @ 的用户头像。
4. 参数中显式写的 `@自己`、`@QQ号` 或图片 URL。
5. 图片数量不足时，按配置自动补当前发送者头像。

注意事项：

- 引用别人发的图片时，只会取被引用消息里的图片，不会把被引用人的头像当作自动补图。
- 缺一张图时，自动补的是当前发送者头像，不是被引用消息的发送者头像。
- 完全没有图片、且表情需要两张图时，会优先使用机器人头像 + 当前发送者头像。
- 如果同时引用图片并 @ 别人，通常会传入“引用图片 + 被 @ 人头像”。
- 部分双图表情对图片顺序很敏感，最终左右/前后位置由 meme-generator 模板定义。
...
确认 `meme_auto_sender_avatar` 已开启，并且运行环境可以访问 `q4.qlogo.cn`。
